...
router.get('/categories', (req, res) => {
  const cats = db.prepare('SELECT * FROM menu_categories WHERE restaurant_id = ? ORDER BY sort_order, name')
...
router.post('/categories', requireRole('admin'), (req, res) => {
  ...
  db.prepare('INSERT INTO menu_categories (id, restaurant_id, name, sort_order) VALUES (?, ?, ?, ?)')
    .run(id, req.user.restaurant_id, name, sort_order || 0);
  res.json(db.prepare('SELECT * FROM menu_categories WHERE id = ?').get(id));
});

router.delete('/categories/:id', requireRole('admin'), (req, res) => {
  db.prepare('DELETE FROM menu_categories WHERE id = ? AND restaurant_id = ?')
...
router.get('/items', (req, res) => {
  ...
  `).all(req.user.restaurant_id);
  res.json(items.map(itemWithCost));
});
...
router.post('/items', requireRole('admin'), (req, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(id, req.user.restaurant_id, category_id || null, recipe_id || null, name, description || null, price);

  const item = db.prepare('SELECT * FROM menu_items WHERE id = ?').get(id);
  const full = itemWithCost(item);
  req.app.get('io').to(req.user.restaurant_id).emit('menu:changed', full);
  res.json(full);
});

router.put('/items/:id', requireRole('admin'), (req, res) => {
  const item = db.prepare('SELECT * FROM menu_items WHERE id = ? AND restaurant_id = ?')
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(name, description, price, category_id, recipe_id, available, req.params.id);

  const updated = db.prepare('SELECT * FROM menu_items WHERE id = ?').get(req.params.id);
  const full = itemWithCost(updated);
